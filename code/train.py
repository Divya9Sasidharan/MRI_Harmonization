...
class train:
    ...
    #os.chdir('../code/Utils')
    model_dir = '../Results/disentangled_latent_space'
    ...
    def fullmodel_one_epoch_run1(epoch=1):
        ...
        for X, Y in training_generator:
            ...
            x_size = X.size()
            batch_size = x_size[0]
            u_x, u_y, d1, d2, d3, d4, o_x, o_y, t_x, t_y, r1, r2, r3, r4, theta_selection_lst_recon = var.complete_model(
                X,
                Y,
                epoch,
        ...
        cosine_similarity_loss_lst_eval, ssim_loss_lst_eval, reconstruction_loss_lst_eval, total_loss_lst_eval = test.test.eval_loss(
            epoch)
        if (epoch % 50 == 0):
            if (epoch > 101):
                modelname = var.model_dir + '/' + "latent_space_harmonization_tau5_without_maxpool_without_transpose_with_activation_" + str(
                    epoch + 0) + '.pth'
                torch.save(complete_model.state_dict(), modelname)
                print("Saving model checkpoints")
            print("epoch: {}".format(epoch + 0))
            print("Losses: {}, {} and {}".format(recon_loss * var.hyperparam1, sim_loss * var.hyperparam2 * var.lmbda,
                                                 total_loss))
            ...
            print("Tau: {}".format(max(var.config["tau_min"], 1 - epoch / hard_epoch)))
            print()
        ...
        wandb.log({"epoch": epoch, "total_loss": sum(abs(x) for x in total_loss_lst) / len(total_loss_lst),
                   "cosine_similarity_loss": sum(abs(x) for x in cosine_similarity_loss_lst) / len(
                       cosine_similarity_loss_lst),
                   "fsim_loss": sum(abs(x) for x in ssim_loss_lst) / len(ssim_loss_lst) * var.hyperparam3,
                   ...
                   "fsim_loss_eval": sum(abs(x) for x in ssim_loss_lst_eval) / len(
                       ssim_loss_lst_eval) * var.hyperparam3,
...
with wandb.init(project="MRI-Harmonization", config=var.config):
    epochs = var.config["epochs"]
    a = []
    b = []
    c = []
    d = []
    wandb.watch(var.complete_model, criterion=nn.MSELoss(), log="all", log_freq=10)
    owd = os.getcwd()
    os.chdir('../code/Utils')
